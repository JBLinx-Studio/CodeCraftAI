...
export class FreeAPIClient implements AIClient {
  ...
  async generateResponse(params: AIRequestParams): Promise<AIServiceResponse> {
    // For free tier, return a simple fallback response
    // Using proper AIServiceResponse structure - checking what properties it actually has
    return {
      text: "I'm a free AI assistant. For full functionality, please configure an API key in settings.",
      usage: {
        prompt_tokens: 0,
        completion_tokens: 0,
        total_tokens: 0
